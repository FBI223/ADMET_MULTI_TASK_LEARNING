...
def compute_mtl_loss(model, logits, yb, tb):
    ...
    for i in torch.unique(tb):
        ...
        l_i = F.mse_loss(logits[mask], yb[mask])
...
def plot_training_curve(history, task_prefix, mode):
    ...
    plt.title(f"MTL Learning Curve - Regression ({mode})")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
...
def plot_smart_results(results_list, metric, filename):
    ...
    g.fig.suptitle(f"Regression: {metric} comparison", y=1.08)
    save_plot(filename)
...
def run_experiment(tasks, feature_mode):
    ...
    y_scalers = {}
    ytr, yva, yte = np.zeros(len(train)), np.zeros(len(val)), np.zeros(len(test))
    for t in tasks:
        idx_t = task_map[t]
        scaler = StandardScaler()
        mask_tr, mask_va, mask_te = (ttr == idx_t), (tva == idx_t), (tte == idx_t)
        ytr[mask_tr] = scaler.fit_transform(train["label"].values[mask_tr].reshape(-1, 1)).flatten()
        yva[mask_va] = scaler.transform(val["label"].values[mask_va].reshape(-1, 1)).flatten()
        yte[mask_te] = scaler.transform(test["label"].values[mask_te].reshape(-1, 1)).flatten()
        y_scalers[t] = scaler

    mtl, history = train_mtl(Xtr, ytr, ttr, Xva, yva, tva, len(tasks))
    plot_training_curve(history, "regression", feature_mode)
    
    mtl.eval()
    with torch.no_grad():
        mtl_preds, _ = mtl(torch.tensor(Xte, dtype=torch.float32), torch.tensor(tte, dtype=torch.long))
    mtl_preds = mtl_preds.numpy()
    
    task_results = []
    for t in tasks:
        idx_t = task_map[t]
        mask_te = tte == idx_t
        te_y_true = test["label"].values[mask_te].astype(float)
        te_preds_mtl = y_scalers[t].inverse_transform(mtl_preds[mask_te].reshape(-1, 1)).flatten()
        
        mtl_r2, mtl_rmse = r2_score(te_y_true, te_preds_mtl), np.sqrt(mean_squared_error(te_y_true, te_preds_mtl))
        ...
        tr_y_stl = tr.label.values.astype(float)
        
        xgb = XGBRegressor(n_estimators=500, max_depth=6, learning_rate=0.05, subsample=0.8).fit(Xtr_t, tr_y_stl)
        xgb_preds = xgb.predict(Xte_t)
        xgb_r2, xgb_rmse = r2_score(te_y_true, xgb_preds), np.sqrt(mean_squared_error(te_y_true, xgb_preds))
        
        rf = RandomForestRegressor(n_estimators=400, n_jobs=-1, random_state=42).fit(Xtr_t, tr_y_stl)
        rf_preds = rf.predict(Xte_t)
        rf_r2, rf_rmse = r2_score(te_y_true, rf_preds), np.sqrt(mean_squared_error(te_y_true, rf_preds))
        
        # Obliczanie zysku z MTL
        gain_r2 = mtl_r2 - max(xgb_r2, rf_r2)
        gain_rmse = min(xgb_rmse, rf_rmse) - mtl_rmse # Dla RMSE mniej znaczy lepiej
        
        task_results.append({
            "mode": feature_mode, "task": task_label,
            "MTL_R2": mtl_r2, "XGB_R2": xgb_r2, "RF_R2": rf_r2, "MTL_Gain_R2": gain_r2,
            "MTL_RMSE": mtl_rmse, "XGB_RMSE": xgb_rmse, "RF_RMSE": rf_rmse, "MTL_Gain_RMSE": gain_rmse
        })
    return task_results
...
if __name__ == "__main__":
    ...
    save_csv(all_results, "results_regression.csv")
    plot_smart_results(all_results, "RMSE", "plot_regression_rmse.png")
    plot_smart_results(all_results, "R2", "plot_regression_r2.png")